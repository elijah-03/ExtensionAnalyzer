...
var ASM_CONSTS = {
  318084: ($0, $1, $2, $3) => { return setTimeout( () => window.inkWasmRegistry[$3].dynCalls["vi"]($1, $2), $0); },  
 318169: ($0) => { clearTimeout($0); },  
 318191: ($0, $1) => { window.inkHostRegistry[$1].setTargetFPS($0); },  
 318240: ($0) => { window.inkHostRegistry[$0].bindScreen(); },  
 318285: ($0, $1) => { window.inkHostRegistry[$1].requestImage(UTF8ToString($0)); },  
 318348: ($0, $1, $2, $3) => { window.inkHostRegistry[$3].setCursor($0, $1, $2); },  
 318402: ($0, $1, $2) => { window.inkHostRegistry[$2].navigateToUri(UTF8ToString($0, $1)); },  
 318470: ($0, $1) => { window.inkHostRegistry[$1].setKeyboardMode($0); },  
 318522: ($0, $1, $2) => { window.inkHostRegistry[$2].onAccessibilityData( new Uint8Array(HEAPU8.subarray($0, $0 + $1))); },  
 318621: ($0) => { return window.inkHostRegistry[$0].shouldPreloadShaders(); },  
 318683: ($0) => { return window.inkHostRegistry[$0].shouldRegisterDocument(); },  
 318747: ($0, $1, $2, $3, $4, $5) => { const buf = window.inkHostRegistry[$5].renderText( HEAPU8.subarray($0, $0 + $1), $2, $3); HEAP8.set(buf, $4); },  
 318861: ($0, $1, $2, $3, $4, $5) => { const name = window.inkHostRegistry[$0].selectMatchingFont( UTF8ToString($1, $2), $3, !!$4, $5); if (name) { const size = lengthBytesUTF8(name) + 1; const tempBuffer = _malloc(size); stringToUTF8(name, tempBuffer, size); return tempBuffer; } return 0; },  
 319117: ($0, $1, $2) => { window.inkHostRegistry[$2].onSceneChange( new Uint8Array(HEAPU8.subarray($0, $0 + $1))); },  
 319210: ($0, $1, $2, $3, $4, $5, $6, $7, $8) => { window.inkHostRegistry[$8].onImageExported( $0, $1, $2, new Uint8ClampedArray(HEAPU8.subarray($3, $3 + $4)), $5, $6, UTF8ToString($7)); },  
 319350: ($0, $1, $2) => { window.inkHostRegistry[$2].onToolEvent( new Uint8Array(HEAPU8.subarray($0, $0 + $1))); },  
 319441: ($0, $1) => { window.inkHostRegistry[$1].onSequencePointReached($0); },  
 319500: ($0, $1, $2) => { window.inkHostRegistry[$2].onFlagChanged($0, !!$1); },  
 319556: ($0) => { window.inkHostRegistry[$0].onCameraChanged(); },  
 319606: ($0, $1) => { window.inkHostRegistry[$1].onCameraMovementStateChanged(!!$0); },  
 319673: ($0, $1) => { window.inkHostRegistry[$1].onBlockingStateChanged(!!$0); },  
 319734: ($0) => { window.inkHostRegistry[$0].onInvisibleLayerActionPrevented(); },  
 319800: ($0, $1, $2) => { window.inkHostRegistry[$2].onPdfLoadSuccess( new Uint8Array(HEAPU8.subarray($0, $0 + $1))); },  
 319896: ($0) => { window.inkHostRegistry[$0].onPdfLoadPasswordRequired(); },  
 319956: ($0, $1, $2) => { window.inkHostRegistry[$2].onPdfLoadFailure(UTF8ToString($0, $1)); },  
 320027: ($0, $1, $2, $3) => { window.inkHostRegistry[$3].onPdfSaveSuccess( $0, new Uint8Array(HEAPU8.subarray($1, $1 + $2))); },  
 320127: ($0, $1, $2, $3) => { window.inkHostRegistry[$3].onPdfSaveFailure($0, UTF8ToString($1, $2)); },  
 320202: ($0, $1) => { window.inkHostRegistry[$0].onPdfUnsupportedFeature($1); },  
 320262: ($0) => { window.inkHostRegistry[$0].onPdfFormChanged(); },  
 320313: ($0, $1, $2, $3, $4, $5, $6, $7) => { window.inkHostRegistry[$0].onPdfFormFieldFocused( $1, new Uint8Array(HEAPU8.subarray($2, $2 + $3)), $4, !!$5, UTF8ToString($6, $7)); },  
 320450: ($0, $1, $2) => { window.inkHostRegistry[$2].onPdfFormTextFieldFocusChanged( true, UTF8ToString($0, $1)); },  
 320542: ($0) => { window.inkHostRegistry[$0].onPdfFormTextFieldFocusChanged(false); },  
 320612: ($0, $1) => { window.inkHostRegistry[$1].onPendingTexturesChanged(!!$0); },  
 320675: ($0, $1, $2) => { const canvas = self.WorkerGlobalScope ? new OffscreenCanvas(128, 128) : document.createElement('canvas'); const gl = canvas.getContext('webgl'); if (gl) { const di = gl.getExtension('WEBGL_debug_renderer_info'); if (di) { stringToUTF8(gl.getParameter(di.UNMASKED_RENDERER_WEBGL), $0, $2); stringToUTF8(gl.getParameter(di.UNMASKED_VENDOR_WEBGL), $1, $2); } } },  
 321037: () => { return !!GLctx },  
 321054: () => { debugger },  
 321067: ($0, $1, $2, $3, $4) => { window.inkHostRegistry[$0].onFatalError(UTF8ToString($1, $2), UTF8ToString($3, $4)); }
};
