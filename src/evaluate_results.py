...
def analyze_results(df):
    ...
    print(
        f"Analyzing {len(y_true)} extensions across {len(thresholds)} unique score thresholds..."
    )
    ...
    print(
        f"Best Threshold:    >= {m['threshold']} (scores of {m['threshold']} or higher are flagged as malicious)"
    )
    print("\n--- Metrics at this threshold ---")
    print(
        f"Precision:         {m['precision']:.4f} (Of all flagged, {(m['precision'] * 100):.1f}% were truly malicious)"
    )
    print(
        f"Recall:            {m['recall']:.4f} (Correctly flagged {m['tp']} out of {m['tp'] + m['fn']} malicious extensions)"
    )
