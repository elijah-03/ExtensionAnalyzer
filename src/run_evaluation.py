...
def main():
    ...
    print(
        f"Found {len(malicious_paths)} malicious and {len(benign_paths)} benign extensions."
    )
        ...
        for i, (path, label) in enumerate(all_paths_with_labels):
            ...
            try:
                ...
                result = subprocess.run(
                    [sys.executable, ANALYZER_SCRIPT, "--path", path],
                    capture_output=True,
                    text=True,
                    encoding="utf-8",
                ...
                if result.returncode != 0:
                    print(
                        f"  -> Error running analyzer on {extension_name} (return code: {result.returncode}):"
                    )
                ...
                try:
                    ...
                    writer.writerow(
                        [extension_id, threat_score, label, threat_report_string]
                    )

                except json.JSONDecodeError:
                    print(
                        f"  -> Error: Could not decode JSON output for {extension_name}."
                    )
                    # Find where the JSON output actually starts
                    json_start_index = result.stdout.find("{")
                    if json_start_index != -1:
                        print(
                            f"  -> Raw STDOUT (from start of JSON): {result.stdout[json_start_index : json_start_index + 200]}..."
                        )
                    else:
                        print(
                            f"  -> Raw STDOUT (no JSON object found): {result.stdout[:200]}..."
                        )
                    print(
                        f"  -> Raw STDERR: {result.stderr[:500]}..."
                    )  # Also print stderr for context
